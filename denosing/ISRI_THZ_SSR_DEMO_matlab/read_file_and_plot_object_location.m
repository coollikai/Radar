...
while (~EXIT_KEY_PRESSED)
    ...
    if BYTES_AVAILABLE_FLAG == 1
        ...
        if (bytevec_cp_len + bytevecAccLen) < bytevec_cp_max_len
            ...
            bytevecAccLen = 0;  %这里有点看不懂，为啥copy以后本身长度设为0
        else
    ...
    if (bytevec_cp_len > 72) && (size(bytevecStr,2) == 1)
        startIdx = strfind(bytevecStr', barker_code);%上文barker_code = char([2 1 4 3 6 5 8 7]);
    else
        startIdx = [];
    end
    if ~isempty(startIdx)%如果不为空
        if startIdx(1) > 1
