 ...
 %
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [] = read_file_and_plot_object_location_with_persistance(dim, record_options, obd_details)
...
% bytevec_cp_max_len  = ceil(bytevecAccLen*8/7);
% bytevec_cp          = zeros(bytevec_cp_max_len,1,'uint8');

barker_code = char([2 1 4 3 6 5 8 7]);
init = 0; 
obd_dat_act = csvread(obd_details.file,5,12); 
obd_dat = obd_dat_act(:,[1 3]); 
obd_dat(:,1) = obd_dat(:,1)/1000;
obd_dat(:,2) = obd_dat(:,2)/3.6;
...
%-------------------- Main Loop ------------------------
while ((~EXIT_KEY_PRESSED) && (bytevecAccLen ~= 0))
    %Read bytes
    tIdleStart = tic;   
    ...
    if(magicOk == 1)
        ...
        timePeriod = (Header.frameNumber - frameInit)*Params(1).frameCfg.framePeriodicity/1000 + obd_details.offset;
        
        idx = find (timePeriod < obd_dat(:,1), 1); 
        if numel(idx) 
            carVel = obd_dat(idx,2);
        else
            carVel = 0;
        end
        
        dist_travelled = carVel*Params(1).frameCfg.framePeriodicity/1000;
        currOffset.x = currOffset.x + sind(obd_details.orientation)*dist_travelled; 
        currOffset.y = currOffset.y + cosd(obd_details.orientation)*dist_travelled; 
        ...
        if sfIdx == 1
            if (clusterObj.numObj > 0) && load_clusters 
                ...
                guiMonitor.clustersCumulativeDataA.x = [guiMonitor.clustersCumulativeDataA.x; clusterObj.x_loc];
                guiMonitor.clustersCumulativeDataA.y = [guiMonitor.clustersCumulativeDataA.y; clusterObj.y_loc];

                set(guiMonitor.clustersPlotHndA, 'Xdata', guiMonitor.clustersCumulativeDataA.x, 'Ydata', guiMonitor.clustersCumulativeDataA.y);
            else
    ...
    if (framePeriod < toc(tIdleStart))
        delay = framePeriod - toc(tIdleStart);
    else
        delay = 0.0002;
    end
    pause(0.001);
...
function [detObj, idx] = getDetObj(bytevec, idx, tlvLen)
    ...
    if tlvLen > 0
        ...
        badDegreeindices  = (abs(degree) > 20);
...
function guiMonitor = gui_initializer(Params, dim)
    ...
    guiMonitor.clustersPlotHnd = plot(inf*ones(6*MAX_NUM_CLUSTERS,1),inf*ones(6*MAX_NUM_CLUSTERS,1),'c', 'LineWidth',2); 
