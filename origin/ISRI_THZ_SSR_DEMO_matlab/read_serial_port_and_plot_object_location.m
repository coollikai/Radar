...
%                         disp(detObj.peak);
        ...
        % Display  以下四个部分是将不同功能的数据存入对应的模块当中，然后在gui_initializer中进行显示
        % 1. Detected objects
        if sfIdx == 1
        ...
        if (trackedObj.numObj > 0) && load_trackers
            ...
            set(guiMonitor.clustersPlotHndA, 'Xdata', trackedObj.clusters_x_loc, 'Ydata', trackedObj.clusters_y_loc);%画绿框
        else
            if sfIdx == 1
...
%     disp(detObj.peakVal);%经过测试，越近的强度越大，但是暂时没有发现金属和非金属的区别，好难啊，，，，
    ...
    %disp(detObj.doppler);%现在测试doppler要么是0要么是-10.4609,好难啊。。。。
...
if tlvLen > 0
    ...
    x_loc = (repmat(x',[1,6])) + x_size'*([-1 1 1 -1 -1 inf]);%(repmat(x',[1,6])) + x_size'*([-1 1 1 -1 -1 inf]);
...
%     disp(detObj.y);disp('dafa');
    %disp(detObj.doppler);%这边计算了正常的doppler
