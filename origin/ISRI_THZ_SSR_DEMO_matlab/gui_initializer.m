...
%   fig=plt.figure()
    ...
    hold on
    axis equal                    
    axis([-dim.max_dist_x/4 dim.max_dist_x/4 0 dim.max_dist_y/4])
    ...
    % Populate the plots. 
    guiMonitor.detectedObjectsPlotHndA = plot(inf,inf,'r.', 'Marker', '.','MarkerSize',20); hold on;%20  redpoint SSR only
    ...
    guiMonitor.clustersPlotHndA = plot(inf*ones(6*MAX_NUM_CLUSTERS,1),inf*ones(6*MAX_NUM_CLUSTERS,1),'g', 'LineWidth',2,'Marker', 'x'); %2追踪
    guiMonitor.clustersPlotHndB = plot(inf*ones(6*MAX_NUM_CLUSTERS,1),inf*ones(6*MAX_NUM_CLUSTERS,1),'b', 'LineWidth',2,'Marker', 'o'); %2融合
    ...
    %title('散布图')
    title('对象方位图','FontSize',12,'FontWeight','bold','color','r','Fontname','STFangSong');
    set(gca,'Color',[1 1 1]);
    ...
    uicontrol('Parent',h_ui,'Units', 'normal ized','Style','checkbox','Position',[(2*step + halfStep + offset) (height+h_thickness+0.002) chkbxwidth thickness],'Value',load_parking_assist,'Callback',@select_parking_assist,'BackgroundColor', [0.8 0.8 0.8]);
